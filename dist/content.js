...
(() => {
  ...
  var map = function(f2) {
    return function(fa) {
      return isNone(fa) ? none2 : some2(f2(fa.value));
    };
  };
  var Functor = {
    URI,
  ...
  var flatMap = /* @__PURE__ */ dual(2, function(ma, f2) {
    return isNone(ma) ? none2 : f2(ma.value);
  });
  var Chain = {
    URI,
    map: _map,
  ...
  var BitapSearch = class {
    ...
    searchIn(text) {
      ...
      this.chunks.forEach(({ pattern, alphabet, startIndex }) => {
        const { isMatch, score: score2, indices } = search(text, pattern, alphabet, {
  ...
  var MOVIE_PERSON_WIDTH = 7;
  var PERSON_MOVIE_WIDTH = 6;
  var makeGraph = (url) => {
    return fetch(url).then((response) => response.json());
  };
  var score = (mPop, pPop, order) => {
    const billingOrder = order === -1 ? 0.5 : order === 0 ? 0.7 : order;
    return (5 * pPop + 3 * mPop) / (billingOrder + 2);
  };
  var year = (movie) => movie.release_date.split("-")[0];
  var formatResult = (result) => {
    const movies = result.movies.map((movie) => `	${movie.title} (${year(movie)})`).join("\n");
    return `via ${result.person.name}:
${movies}`;
  };
  var recommendations = (battle, movie, graph) => {
    const peopleIds = (0, import_get.default)(graph.movie_credits, movie.id);
    const movieCredits = (0, import_keys.default)(peopleIds).map((id) => {
      return { id, order: (0, import_get.default)(peopleIds, id) };
    });
    const people = movieCredits.map((credit) => {
      return { ...(0, import_get.default)(graph.people, credit.id), order: credit.order };
    });
    const sortedPeople = (0, import_sortBy.default)(
      people,
      (person) => -1 * score(movie.popularity, person.popularity, person.order)
    ).slice(0, MOVIE_PERSON_WIDTH);
    return sortedPeople.map((person) => {
      const movieIds = (0, import_get.default)(graph.people_credits, person.id);
      const peopleCredits = (0, import_keys.default)(movieIds).map((id) => {
        return { id, order: (0, import_get.default)(movieIds, id) };
      });
      const movies = peopleCredits.map((credit) => {
        return { ...(0, import_get.default)(graph.movies, credit.id), order: credit.order };
      }).filter((m) => !battle.usedMovieIds.includes(m.id));
      const sortedMovies = (0, import_sortBy.default)(
        movies,
        (movie2) => -1 * score(movie2.popularity, person.popularity, movie2.order)
      ).slice(0, PERSON_MOVIE_WIDTH);
      return { person, movies: sortedMovies };
    });
  };
  var makeFuse = (indexUrl, graph) => {
    const options = { keys: ["title", "year"] };
    const movies = (0, import_values.default)(graph.movies).map((movie) => {
      return { ...movie, year: movie.release_date ? movie.release_date.split("-")[0] : "n/a" };
    });
    return fetch(indexUrl).then((response) => response.json()).then((index) => Fuse.parseIndex(index)).then((index) => new Fuse(movies, options, index));
  };
  var search2 = (fuse, battleMovie) => {
  ...
  // src/content.ts
  var handleNewMovie = (battle, graph, movie) => {
  ...
  var awaitNewMovies = (g2, f2) => (board) => {
      ...
      if (mutations.length > 2) {
        ...
        if (topMovie !== lastMovie(battle)) {
          map((battleMovie) => {
            g2.then(
              (graph) => f2.then((fuse) => {
                const movie = search2(fuse, battleMovie);
  ...
  var g = makeGraph(chrome.runtime.getURL("static/graph.json"));
  var f = g.then((graph) => makeFuse(chrome.runtime.getURL("static/fuseIndex.json"), graph));
  var run = () => awaitBattle().then(awaitNewMovies(g, f));
  run();
})();
